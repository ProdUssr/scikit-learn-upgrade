...
from sklearn_upgrade._config import config_context, get_config
from sklearn_upgrade.exceptions import InconsistentVersionWarning
from sklearn_upgrade.utils._metadata_requests import _MetadataRequester, _routing_enabled
from sklearn_upgrade.utils._missing import is_pandas_na, is_scalar_nan
from sklearn_upgrade.utils._param_validation import validate_parameter_constraints
from sklearn_upgrade.utils._repr_html.base import ReprHTMLMixin, _HTMLDocumentationLinkMixin
from sklearn_upgrade.utils._repr_html.estimator import estimator_html_repr
from sklearn_upgrade.utils._repr_html.fitted_attributes import AttrsDict
from sklearn_upgrade.utils._repr_html.params import ParamsDict
from sklearn_upgrade.utils._set_output import _SetOutputMixin
from sklearn_upgrade.utils._tags import (
    ClassifierTags,
    RegressorTags,
    Tags,
    TargetTags,
...
from sklearn_upgrade.utils.fixes import _IS_32BIT
from sklearn_upgrade.utils.validation import (
...
def clone(estimator, *, safe=True):
    ...
    .. versionchanged:: 1.3
        Delegates to `estimator.__sklearn_upgrade_clone__` if the method exists.
    ...
    safe : bool, default=True
        ...
        that are not estimators. Ignored if `estimator.__sklearn_upgrade_clone__`
        exists.
    ...
    Examples
    --------
    >>> from sklearn_upgrade.base import clone
    >>> from sklearn_upgrade.linear_model import LogisticRegression
    >>> X = [[-1, 0], [0, 1], [0, -1], [1, 0]]
    >>> y = [0, 0, 1, 1]
    ...
    False
    """
    if hasattr(estimator, "__sklearn_upgrade_clone__") and not inspect.isclass(estimator):
        return estimator.__sklearn_upgrade_clone__()
    return _clone_parametrized(estimator, safe=safe)


def _clone_parametrized(estimator, *, safe=True):
    """Default implementation of clone. See :func:`sklearn_upgrade.base.clone` for details."""
    ...
    # _sklearn_upgrade_output_config is used by `set_output` to configure the output
    # container of an estimator.
    if hasattr(estimator, "_sklearn_upgrade_output_config"):
        new_object._sklearn_upgrade_output_config = copy.deepcopy(
            estimator._sklearn_upgrade_output_config
        )
    return new_object
...
class BaseEstimator(ReprHTMLMixin, _HTMLDocumentationLinkMixin, _MetadataRequester):
    ...
    --------
    >>> import numpy as np
    >>> from sklearn_upgrade.base import BaseEstimator
    ...
    def set_params(self, **params):
        ...
        (such as :class:`~sklearn_upgrade.pipeline.Pipeline`). The latter have
    ...
    def __sklearn_upgrade_clone__(self):
    ...
    def __repr__(self, N_CHAR_MAX=700):
        ...
        from sklearn_upgrade.utils._pprint import _EstimatorPrettyPrinter
    ...
    def __getstate__(self):
        if getattr(self, "__slots__", None):
            raise TypeError(
                ...
                "`sklearn_upgrade.base.BaseEstimator`."
            )
        ...
        if type(self).__module__.startswith("sklearn_upgrade."):
            return dict(state.items(), _sklearn_upgrade_version=__version__)
        else:
            return state

    def __setstate__(self, state):
        if type(self).__module__.startswith("sklearn_upgrade."):
            pickle_version = state.pop("_sklearn_upgrade_version", "pre-0.18")
            if pickle_version != __version__:
                warnings.warn(
                    InconsistentVersionWarning(
                        estimator_name=self.__class__.__name__,
                        current_sklearn_upgrade_version=__version__,
                        original_sklearn_upgrade_version=pickle_version,
                    ),
                )
        try:
    ...
    def __sklearn_upgrade_tags__(self):
        return Tags(
            estimator_type=None,
...
class ClassifierMixin:
    ...
    - `score` method that default to :func:`~sklearn_upgrade.metrics.accuracy_score`.
    ...
    --------
    >>> import numpy as np
    >>> from sklearn_upgrade.base import BaseEstimator, ClassifierMixin
    ...
    def __sklearn_upgrade_tags__(self):
        tags = super().__sklearn_upgrade_tags__()
    ...
    def score(self, X, y, sample_weight=None):
        ...
        """
        from sklearn_upgrade.metrics import accuracy_score
...
class RegressorMixin:
    ...
    - `score` method that default to :func:`~sklearn_upgrade.metrics.r2_score`.
    - enforce that `fit` requires `y` to be passed through the `requires_y` tag,
    ...
    --------
    >>> import numpy as np
    >>> from sklearn_upgrade.base import BaseEstimator, RegressorMixin
    >>> # Mixin classes should always be on the left-hand side for a correct MRO
    ...
    def score(self, X, y, sample_weight=None):
        ...
        with default value of :func:`~sklearn_upgrade.metrics.r2_score`.
        This influences the ``score`` method of all the multioutput
        regressors (except for
        :class:`~sklearn_upgrade.multioutput.MultiOutputRegressor`).
        """

        from sklearn_upgrade.metrics import r2_score
...
class ClusterMixin:
    ...
    --------
    >>> import numpy as np
    >>> from sklearn_upgrade.base import BaseEstimator, ClusterMixin
...
class BiclusterMixin:
    ...
    --------
    >>> import numpy as np
    >>> from sklearn_upgrade.base import BaseEstimator, BiclusterMixin
...
class TransformerMixin(_SetOutputMixin):
    ...
    --------
    >>> import numpy as np
    >>> from sklearn_upgrade.base import BaseEstimator, TransformerMixin
...
class OneToOneFeatureMixin:
    ...
    and output features, such as :class:`~sklearn_upgrade.preprocessing.StandardScaler`.

    Examples
    --------
    >>> import numpy as np
    >>> from sklearn_upgrade.base import OneToOneFeatureMixin, BaseEstimator
...
class ClassNamePrefixFeaturesOutMixin:
    ...
    names out, such as :class:`~sklearn_upgrade.decomposition.PCA`. For example, if
    :class:`~sklearn_upgrade.decomposition.PCA` outputs 3 features, then the generated feature
    ...
    --------
    >>> import numpy as np
    >>> from sklearn_upgrade.base import ClassNamePrefixFeaturesOutMixin, BaseEstimator
...
class DensityMixin:
    ...
    Examples
    --------
    >>> from sklearn_upgrade.base import DensityMixin
...
class OutlierMixin:
    ...
    --------
    >>> import numpy as np
    >>> from sklearn_upgrade.base import BaseEstimator, OutlierMixin
...
class MetaEstimatorMixin:
    ...
    Examples
    --------
    >>> from sklearn_upgrade.base import MetaEstimatorMixin
    >>> from sklearn_upgrade.datasets import load_iris
    >>> from sklearn_upgrade.linear_model import LogisticRegression
...
def is_classifier(estimator):
    ...
    Examples
    --------
    >>> from sklearn_upgrade.base import is_classifier
    >>> from sklearn_upgrade.cluster import KMeans
    >>> from sklearn_upgrade.svm import SVC, SVR
    >>> classifier = SVC()
    >>> regressor = SVR()
    >>> kmeans = KMeans()
...
def is_regressor(estimator):
    ...
    Examples
    --------
    >>> from sklearn_upgrade.base import is_regressor
    >>> from sklearn_upgrade.cluster import KMeans
    >>> from sklearn_upgrade.svm import SVC, SVR
    >>> classifier = SVC()
    >>> regressor = SVR()
...
def is_clusterer(estimator):
    ...
    Examples
    --------
    >>> from sklearn_upgrade.base import is_clusterer
    >>> from sklearn_upgrade.cluster import KMeans
    >>> from sklearn_upgrade.svm import SVC, SVR
    >>> classifier = SVC()
    >>> regressor = SVR()
