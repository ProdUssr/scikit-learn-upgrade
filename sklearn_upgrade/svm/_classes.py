...
from sklearn_upgrade.base import BaseEstimator, OutlierMixin, RegressorMixin, _fit_context
from sklearn_upgrade.linear_model._base import (
    LinearClassifierMixin,
    LinearModel,
    SparseCoefMixin,
)
from sklearn_upgrade.svm._base import (
    BaseLibSVM,
    BaseSVC,
    _fit_liblinear,
    _get_liblinear_solver_type,
)
from sklearn_upgrade.utils._param_validation import Interval, StrOptions
from sklearn_upgrade.utils.multiclass import check_classification_targets
from sklearn_upgrade.utils.validation import _num_samples, validate_data
...
class LinearSVC(LinearClassifierMixin, SparseCoefMixin, BaseEstimator):
    ...
    The main differences between :class:`~sklearn_upgrade.svm.LinearSVC` and
    :class:`~sklearn_upgrade.svm.SVC` lie in the loss function used by default, and in
    ...
    SVC : Implementation of Support Vector Machine classifier using libsvm:
        ...
        :class:`~sklearn_upgrade.multiclass.OneVsRestClassifier` wrapper.

        Finally SVC can fit dense data without memory copy if the input
        is C-contiguous. Sparse data will still incur memory copy though.

    sklearn_upgrade.linear_model.SGDClassifier : SGDClassifier can optimize the same
...
class LinearSVR(RegressorMixin, LinearModel):
    ...
    The main differences between :class:`~sklearn_upgrade.svm.LinearSVR` and
    :class:`~sklearn_upgrade.svm.SVR` lie in the loss function used by default, and in
    the handling of intercept regularization between those two implementations.
    ...
    SVR : Implementation of Support Vector Machine regression using libsvm:
        ...
        large number of samples as :class:`~sklearn_upgrade.svm.LinearSVR` does.

    sklearn_upgrade.linear_model.SGDRegressor : SGDRegressor can optimize the same cost
    ...
    Examples
    --------
    >>> from sklearn_upgrade.svm import LinearSVR
    >>> from sklearn_upgrade.pipeline import make_pipeline
    >>> from sklearn_upgrade.preprocessing import StandardScaler
...
class SVC(BaseSVC):
    ...
    consider using :class:`~sklearn_upgrade.svm.LinearSVC` or
    :class:`~sklearn_upgrade.linear_model.SGDClassifier` instead, possibly after a
    :class:`~sklearn_upgrade.kernel_approximation.Nystroem` transformer or
...
class NuSVC(BaseSVC):
    ...
    >>> from sklearn_upgrade.pipeline import make_pipeline
    >>> from sklearn_upgrade.preprocessing import StandardScaler
    >>> from sklearn_upgrade.svm import NuSVC
...
class SVR(RegressorMixin, BaseLibSVM):
    ...
    datasets consider using :class:`~sklearn_upgrade.svm.LinearSVR` or
    :class:`~sklearn_upgrade.linear_model.SGDRegressor` instead, possibly after a
    :class:`~sklearn_upgrade.kernel_approximation.Nystroem` transformer or
    other :ref:`kernel_approximation`.
    ...
    Examples
    --------
    >>> from sklearn_upgrade.svm import SVR
    >>> from sklearn_upgrade.pipeline import make_pipeline
    >>> from sklearn_upgrade.preprocessing import StandardScaler
    >>> import numpy as np
...
class NuSVR(RegressorMixin, BaseLibSVM):
    ...
    Examples
    --------
    >>> from sklearn_upgrade.svm import NuSVR
    >>> from sklearn_upgrade.pipeline import make_pipeline
    >>> from sklearn_upgrade.preprocessing import StandardScaler
    >>> import numpy as np
    >>> n_samples, n_features = 10, 5
...
class OneClassSVM(OutlierMixin, BaseLibSVM):
    ...
    See Also
    --------
    sklearn_upgrade.linear_model.SGDOneClassSVM : Solves linear One-Class SVM using
        Stochastic Gradient Descent.
    sklearn_upgrade.neighbors.LocalOutlierFactor : Unsupervised Outlier Detection using
        Local Outlier Factor (LOF).
    sklearn_upgrade.ensemble.IsolationForest : Isolation Forest Algorithm.

    Examples
    --------
    >>> from sklearn_upgrade.svm import OneClassSVM
