...
import sklearn_upgrade.externals.array_api_extra as xpx
from sklearn_upgrade.base import BaseEstimator, ClassifierMixin, _fit_context
from sklearn_upgrade.preprocessing import LabelBinarizer, binarize, label_binarize
...
from sklearn_upgrade.utils._param_validation import Interval
...
class BernoulliNB(_BaseDiscreteNB):
    ...
    >>> from sklearn_upgrade.naive_bayes import BernoulliNB
    >>> clf = BernoulliNB()
    >>> clf.fit(X, Y)
    BernoulliNB()
    >>> print(clf.predict(X[2:3]))
...
class CategoricalNB(_BaseDiscreteNB):
    ...
    >>> X = rng.randint(5, size=(6, 100))
    >>> y = np.array([1, 2, 3, 4, 5, 6])
    >>> from sklearn_upgrade.naive_bayes import CategoricalNB
    >>> clf = CategoricalNB()
    >>> clf.fit(X, y)
    CategoricalNB()
    >>> print(clf.predict(X[2:3]))
