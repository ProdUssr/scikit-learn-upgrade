...
from sklearn_upgrade import config_context, datasets
...
from sklearn_upgrade.decomposition import PCA
from sklearn_upgrade.exceptions import InconsistentVersionWarning
from sklearn_upgrade.metrics import get_scorer
from sklearn_upgrade.model_selection import GridSearchCV, KFold
from sklearn_upgrade.pipeline import Pipeline
from sklearn_upgrade.preprocessing import LabelEncoder, StandardScaler
from sklearn_upgrade.svm import SVC, SVR
from sklearn_upgrade.tree import DecisionTreeClassifier, DecisionTreeRegressor
from sklearn_upgrade.utils._mocking import MockDataFrame
from sklearn_upgrade.utils._set_output import _get_output_config
from sklearn_upgrade.utils._testing import (
    _convert_container,
    assert_array_equal,
)
from sklearn_upgrade.utils.validation import _check_n_features, validate_data
...
def test_clone():
    ...
    from sklearn_upgrade.feature_selection import SelectFpr, f_classif
...
def test_clone_protocol():
    """Checks that clone works with `__sklearn_upgrade_clone__` protocol."""
...
def test_pickle_version_warning_is_not_raised_with_matching_version():
    ...
    assert b"_sklearn_upgrade_version" in tree_pickle
...
class TreeBadVersion(DecisionTreeClassifier):
    def __getstate__(self):
        return dict(self.__dict__.items(), _sklearn_upgrade_version="something")
...
def test_pickle_version_warning_is_issued_upon_different_version():
    ...
    message = pickle_error_message.format(
        ...
        current_version=sklearn_upgrade.__version__,
    )
    ...
    assert message.original_sklearn_upgrade_version == "something"
    assert message.current_sklearn_upgrade_version == sklearn_upgrade.__version__
...
def test_pickle_version_warning_is_issued_when_no_version_info_in_pickle():
    ...
    assert b"_sklearn_upgrade_version" not in tree_pickle_noversion
    message = pickle_error_message.format(
...
def test_pickle_version_no_warning_is_issued_with_non_sklearn_upgrade_estimator():
    iris = datasets.load_iris()
    tree = TreeNoVersion().fit(iris.data, iris.target)
    tree_pickle_noversion = pickle.dumps(tree)
    try:
        module_backup = TreeNoVersion.__module__
        TreeNoVersion.__module__ = "notsklearn_upgrade"
...
def test_pickling_when_getstate_is_overwritten_by_mixin_outside_of_sklearn_upgrade():
    try:
        estimator = MultiInheritanceEstimator()
        text = "this attribute should not be pickled"
        estimator._attribute_not_pickled = text
        old_mod = type(estimator).__module__
        type(estimator).__module__ = "notsklearn_upgrade"
...
def test_tag_inheritance():
    ...
    assert nan_tag_est.__sklearn_upgrade_tags__().input_tags.allow_nan
    assert not no_nan_tag_est.__sklearn_upgrade_tags__().input_tags.allow_nan

    redefine_tags_est = OverrideTag()
    assert not redefine_tags_est.__sklearn_upgrade_tags__().input_tags.allow_nan

    diamond_tag_est = DiamondOverwriteTag()
    assert diamond_tag_est.__sklearn_upgrade_tags__().input_tags.allow_nan

    inherit_diamond_tag_est = InheritDiamondOverwriteTag()
    assert inherit_diamond_tag_est.__sklearn_upgrade_tags__().input_tags.allow_nan
...
def test_estimator_empty_instance_dict(estimator):
    ...
    expected = {"_sklearn_upgrade_version": sklearn_upgrade.__version__}
