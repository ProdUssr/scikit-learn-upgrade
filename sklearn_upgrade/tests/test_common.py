"""
General tests for all estimators in sklearn_upgrade.
"""
...
from sklearn_upgrade.pipeline import FeatureUnion, make_pipeline
...
def test_import_all_consistency():
    ...
    for modname in submods + ["sklearn_upgrade"]:
        if ".tests." in modname or "sklearn_upgrade.externals" in modname:
            continue
        # Avoid test suite depending on build dependencies, for example Cython
        if "sklearn_upgrade._build_utils" in modname:
            continue
...
def test_root_import_all_completeness():
    ...
    for _, modname, _ in pkgutil.walk_packages(
        path=sklearn_upgrade_path, onerror=lambda _: None
    ):
        if "." in modname or modname.startswith("_") or modname in EXCEPTIONS:
            continue
        assert modname in sklearn_upgrade.__all__
...
def test_all_tests_are_importable():
    ...
    resource_modules = {
        "sklearn_upgrade.datasets.data",
        "sklearn_upgrade.datasets.descr",
        "sklearn_upgrade.datasets.images",
    }
    sklearn_upgrade_path = [os.path.dirname(sklearn_upgrade.__file__)]
    lookup = {
        name: ispkg
        for _, name, ispkg in pkgutil.walk_packages(sklearn_upgrade_path, prefix="sklearn_upgrade.")
    }
    missing_tests = [
        name
