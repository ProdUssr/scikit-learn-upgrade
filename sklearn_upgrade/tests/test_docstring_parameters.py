...
from sklearn_upgrade.experimental import (
...
from sklearn_upgrade.utils._test_common.instance_generator import _construct_instances
from sklearn_upgrade.utils._testing import (
    _get_func_name,
    check_docstring_parameters,
    ignore_warnings,
)
from sklearn_upgrade.utils.deprecation import _is_deprecated
...
with warnings.catch_warnings():
    ...
    PUBLIC_MODULES = set(
        [
            pckg[1]
            for pckg in walk_packages(prefix="sklearn_upgrade.", path=sklearn_upgrade_path)
            if not any(
                substr in pckg[1] for substr in ["._", ".tests.", "sklearn_upgrade.externals"]
            )
        ]
    )
...
_DOCSTRING_IGNORES = [
    "sklearn_upgrade.utils.deprecation.load_mlcomp",
    "sklearn_upgrade.pipeline.make_pipeline",
    "sklearn_upgrade.pipeline.make_union",
    "HalfBinomialLoss",
]
...
def test_docstring_parameters():
    ...
    for name in PUBLIC_MODULES:
        if name.endswith(".conftest"):
            ...
            continue
        if name == "sklearn_upgrade.utils.fixes":
        ...
        classes = [cls for cls in classes if cls[1].__module__.startswith("sklearn_upgrade")]
...
def test_fit_docstring_attributes(name, Estimator):
    ...
    if est.__sklearn_upgrade_tags__().target_tags.one_d_labels:
        est.fit(y)
    elif est.__sklearn_upgrade_tags__().target_tags.two_d_labels:
        est.fit(np.c_[y, y])
    elif est.__sklearn_upgrade_tags__().input_tags.three_d_array:
