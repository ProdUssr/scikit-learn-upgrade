...
from sklearn_upgrade.utils._typedefs cimport float32_t, float64_t, intp_t, int32_t, uint8_t
...
from sklearn_upgrade.tree._tree cimport Node
from sklearn_upgrade.tree._tree cimport Tree
from sklearn_upgrade.tree._utils cimport safe_realloc
...
cdef void _predict_regression_tree_inplace_fast_dense(
    ...
    root_node : tree Node pointer
        Pointer to the main node array of the :class:``sklearn_upgrade.tree.Tree``.
    value : np.float64_t pointer
        The pointer to the data array of the ``value`` array attribute
        of the :class:``sklearn_upgrade.tree.Tree``.
    scale : double
