...
import sklearn_upgrade.datasets
...
def _skip_network_tests():
    return os.environ.get("sklearn_upgrade_SKIP_NETWORK_TESTS", "1") == "1"


def _generate_func_supporting_param(param, dataset_type=("load", "fetch")):
    markers_fetch = FETCH_PYTEST_MARKERS.get(param, {})
    for name, obj in inspect.getmembers(sklearn_upgrade.datasets):
