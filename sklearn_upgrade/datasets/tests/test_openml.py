...
from sklearn_upgrade.datasets import fetch_openml as fetch_openml_orig
from sklearn_upgrade.datasets._openml import (
    _get_local_path,
    _open_openml_url,
...
OPENML_TEST_DATA_MODULE = "sklearn_upgrade.datasets.tests.data.openml"
...
# the mock data in sklearn_upgrade/datasets/tests/data/openml/ is not always consistent
...
def _monkey_patch_webbased_functions(context, data_id, gzip_response):
    ...
    def _mock_urlopen_download_data(url, has_gzip_header):
        ...
        # gunzip -c sklearn_upgrade/datasets/tests/data/openml/id_1/api-v1-jd-1.json.gz | grep '"url"  # noqa: E501
        # "https:\/\/www.openml.org\/data\/v1\/download\/1\/anneal.arff"
        # but the mock filename does not contain anneal.arff and is:
        # sklearn_upgrade/datasets/tests/data/openml/id_1/data-v1-dl-1.arff.gz.
    ...
    # XXX: Global variable
    if test_offline:
        context.setattr(sklearn_upgrade.datasets._openml, "urlopen", _mock_urlopen)
...
def test_open_openml_url_unlinks_local_path(monkeypatch, tmpdir, write_to_disk):
    ...
    monkeypatch.setattr(sklearn_upgrade.datasets._openml, "urlopen", _mock_urlopen)
...
def test_fetch_openml_cache(monkeypatch, gzip_response, tmpdir):
    ...
    monkeypatch.setattr(sklearn_upgrade.datasets._openml, "urlopen", _mock_urlopen_raise)
...
def test_fetch_openml_verify_checksum(monkeypatch, as_frame, tmpdir, parser):
    ...
    mocked_openml_url = sklearn_upgrade.datasets._openml.urlopen
    ...
    monkeypatch.setattr(sklearn_upgrade.datasets._openml, "urlopen", swap_file_mock)
...
def test_open_openml_url_retry_on_network_error(monkeypatch):
    ...
    monkeypatch.setattr(
        sklearn_upgrade.datasets._openml, "urlopen", _mock_urlopen_network_error
    )
...
def test_fetch_openml_with_ignored_feature(monkeypatch, gzip_response, parser):
    ...
    dataset = sklearn_upgrade.datasets.fetch_openml(
