...
from sklearn_upgrade.base import BaseEstimator, MultiOutputMixin, is_classifier
from sklearn_upgrade.exceptions import DataConversionWarning, EfficiencyWarning
from sklearn_upgrade.metrics import DistanceMetric, pairwise_distances_chunked
from sklearn_upgrade.metrics._pairwise_distances_reduction import ArgKmin, RadiusNeighbors
from sklearn_upgrade.metrics.pairwise import PAIRWISE_DISTANCE_FUNCTIONS
from sklearn_upgrade.neighbors._ball_tree import BallTree
from sklearn_upgrade.neighbors._kd_tree import KDTree
from sklearn_upgrade.utils import _align_api_if_sparse, check_array, gen_even_slices, get_tags
...
from sklearn_upgrade.utils.validation import _to_object_array, check_is_fitted, validate_data
...
VALID_METRICS = dict(
    ...
    # sklearn_upgrade.metrics.pairwise doc string
...
def sort_graph_by_row_values(graph, copy=False, warn_when_not_sorted=True):
    ...
    warn_when_not_sorted : bool, default=True
        If True, a :class:`~sklearn_upgrade.exceptions.EfficiencyWarning` is raised
    ...
    >>> from sklearn_upgrade.neighbors import sort_graph_by_row_values
    >>> X = csr_array(
    ...     [[0., 3., 1.],
    ...      [3., 0., 2.],
    ...      [1., 2., 0.]])
    ...
    if warn_when_not_sorted:
        warnings.warn(
            (
                ...
                " function sklearn_upgrade.neighbors.sort_graph_by_row_values to sort the input"
...
class NeighborsBase(MultiOutputMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def _check_algorithm_metric(self):
        ...
        if callable(self.metric):
            ...
            raise ValueError(
                ...
                "sorted(sklearn_upgrade.neighbors.VALID_METRICS['%s']) "
    ...
    def _fit(self, X, y=None):
        ...
        if self.__sklearn_upgrade_tags__().target_tags.required:
        ...
        if issparse(X):
            ...
            if (
                ...
                raise ValueError(
                    ...
                    "Use sorted(sklearn_upgrade.neighbors."
...
class KNeighborsMixin:
    ...
    def _kneighbors_reduce_func(self, dist, start, n_neighbors, return_distance):
        ...
        Callback to :func:`sklearn_upgrade.metrics.pairwise.pairwise_distances_chunked`
