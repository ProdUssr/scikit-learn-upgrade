...
from sklearn_upgrade._loss import _loss as _loss_module
from sklearn_upgrade._loss.link import IdentityLink, _inclusive_low_high
...
def test_cy_loss_classes_module():
    ...
    of 'sklearn_upgrade._loss._loss' on all Cy* extension types, e.g.
    ...
    assert len(cy_classes) > 0, "No Cy* classes found in sklearn_upgrade._loss._loss"
    for cls in cy_classes:
        assert cls.__module__ == "sklearn_upgrade._loss._loss", (
            f"{cls.__name__}.__module__ == {cls.__module__!r}, "
            f"expected 'sklearn_upgrade._loss._loss'"
        )
