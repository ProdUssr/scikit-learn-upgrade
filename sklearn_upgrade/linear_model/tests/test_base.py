...
from sklearn_upgrade.datasets import load_iris, make_regression, make_sparse_uncorrelated
...
from sklearn_upgrade.utils._array_api import get_namespace_and_device, move_estimator_to
...
def test_linear_regression_vs_lstsq(dtype):
    ...
    coef_sklearn_upgrade = LinearRegression(fit_intercept=False).fit(X, y).coef_

    rmse_scipy = np.linalg.norm(y - X @ coef_scipy)
    rmse_sklearn_upgrade = np.linalg.norm(y - X @ coef_sklearn_upgrade)
    assert rmse_sklearn_upgrade == pytest.approx(rmse_scipy, rel=1e-6)
...
def test_predict_proba_lr_large_values():
    ...
    class MockClassifier(LinearClassifierMixin, BaseEstimator):
        ...
        def fit(self, X, y):
            self.__sklearn_upgrade_is_fitted__ = True
