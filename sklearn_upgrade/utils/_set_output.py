...
def _get_adapter_from_container(container):
    ...
    See :class:`sklearn_upgrade.utils._set_output.ContainerAdapterProtocol` for more
    details.
    """
...
def _get_output_config(method, estimator=None):
    ...
    config : dict
        ...
        - "dense": specifies the dense container for `method`. This can be
          `"default"` or `"pandas"`.
    """
    est_sklearn_upgrade_output_config = getattr(estimator, "_sklearn_upgrade_output_config", {})
    if method in est_sklearn_upgrade_output_config:
        dense_config = est_sklearn_upgrade_output_config[method]
    else:
...
def _auto_wrap_is_configured(estimator):
    ...
    `_SetOutputMixin` sets `_sklearn_upgrade_auto_wrap_output_keys` to `set()` if auto wrapping
    is manually disabled.
    """
    auto_wrap_output_keys = getattr(estimator, "_sklearn_upgrade_auto_wrap_output_keys", set())
    return (
...
class _SetOutputMixin:
    ...
    def __init_subclass__(cls, auto_wrap_output_keys=("transform",), **kwargs):
        ...
        if auto_wrap_output_keys is None:
            cls._sklearn_upgrade_auto_wrap_output_keys = set()
            return
        ...
        for method, key in method_to_key.items():
            if not hasattr(cls, method) or key not in auto_wrap_output_keys:
                continue
            cls._sklearn_upgrade_auto_wrap_output_keys.add(key)
