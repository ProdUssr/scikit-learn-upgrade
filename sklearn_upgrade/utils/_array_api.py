...
from sklearn_upgrade._config import get_config
from sklearn_upgrade.externals import array_api_compat
from sklearn_upgrade.externals import array_api_extra as xpx
from sklearn_upgrade.externals.array_api_compat import numpy as np_compat
from sklearn_upgrade.utils._dataframe import is_df_or_series
...
_NUMPY_NAMESPACE_NAMES = {"numpy", "sklearn_upgrade.externals.array_api_compat.numpy"}
...
def _check_array_api_dispatch(array_api_dispatch):
    ...
    if os.environ.get("SCIPY_ARRAY_API") != "1":
        raise RuntimeError(
            ...
            "before importing sklearn_upgrade or scipy. More details at: "
...
def get_namespace(
    *arrays, remove_none=True, remove_types=REMOVE_TYPES_DEFAULT, xp=None
    ...
    Namespace support is not enabled by default. To enabled it call:
      ...
      sklearn_upgrade.set_config(array_api_dispatch=True)

    or:

      with sklearn_upgrade.config_context(array_api_dispatch=True):
          # your code here
...
def _is_xp_namespace(xp, name):
    return xp.__name__ in (
        ...
        f"sklearn_upgrade.externals.array_api_compat.{name}",
    )
...
def _estimator_with_converted_arrays(estimator, converter):
    ...
    if hasattr(estimator, "__sklearn_upgrade_array_api_convert__") and not inspect.isclass(
        estimator
    ):
        return estimator.__sklearn_upgrade_array_api_convert__(converter)
...
def check_same_namespace(X, estimator, *, attribute, method):
    ...
    raise ValueError(
        ...
        "'from sklearn_upgrade.utils._array_api import move_estimator_to; "
...
def _count_nonzero(X, axis=None, sample_weight=None, xp=None, device=None):
    """A variant of `sklearn_upgrade.utils.sparsefuncs.count_nonzero` for the Array API.
