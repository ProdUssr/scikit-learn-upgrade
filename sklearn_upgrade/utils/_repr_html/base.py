...
class _HTMLDocumentationLinkMixin:
    ...
    - `_doc_link_module`: it corresponds to the root module (e.g. `sklearn_upgrade`). Using this
      mixin, the default value is `sklearn_upgrade`.
    ...
    :func:`sklearn_upgrade.utils.estimator_html_repr` to generate a link to the API
    ...
    _doc_link_module = "sklearn_upgrade"
    _doc_link_url_param_generator = None

    @property
    def _doc_link_template(self):
        sklearn_upgrade_version = parse_version(__version__)
        if sklearn_upgrade_version.dev is None:
            version_url = f"{sklearn_upgrade_version.major}.{sklearn_upgrade_version.minor}"
        else:
            version_url = "dev"
        return getattr(
            self,
