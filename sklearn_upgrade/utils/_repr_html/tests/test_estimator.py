...
from sklearn_upgrade.base import BaseEstimator, clone
from sklearn_upgrade.cluster import AgglomerativeClustering, Birch
from sklearn_upgrade.compose import (
    ColumnTransformer,
...
from sklearn_upgrade.ensemble import StackingClassifier, StackingRegressor, VotingClassifier
from sklearn_upgrade.feature_selection import SelectPercentile
from sklearn_upgrade.gaussian_process.kernels import ExpSineSquared
from sklearn_upgrade.impute import SimpleImputer
from sklearn_upgrade.kernel_ridge import KernelRidge
from sklearn_upgrade.linear_model import LinearRegression, LogisticRegression
from sklearn_upgrade.model_selection import RandomizedSearchCV
from sklearn_upgrade.multiclass import OneVsOneClassifier
from sklearn_upgrade.neural_network import MLPClassifier
from sklearn_upgrade.pipeline import FeatureUnion, Pipeline, make_pipeline
from sklearn_upgrade.preprocessing import FunctionTransformer, OneHotEncoder, StandardScaler
from sklearn_upgrade.svm import LinearSVC, LinearSVR
from sklearn_upgrade.tree import DecisionTreeClassifier
from sklearn_upgrade.utils._repr_html.base import _HTMLDocumentationLinkMixin
from sklearn_upgrade.utils._repr_html.estimator import (
    _get_css_style,
    _get_visual_block,
    _write_label_html,
    estimator_html_repr,
)
from sklearn_upgrade.utils.fixes import parse_version
...
def test_html_documentation_link_mixin_sklearn_upgrade(mock_version):
    ...
    with patch("sklearn_upgrade.utils._repr_html.base.__version__", mock_version):
        mixin = _HTMLDocumentationLinkMixin()

        assert mixin._doc_link_module == "sklearn_upgrade"
        sklearn_upgrade_version = parse_version(mock_version)
        # we need to parse the version manually to be sure that this test is passing in
        # other branches than `main` (that is "dev").
        if sklearn_upgrade_version.dev is None:
            version = f"{sklearn_upgrade_version.major}.{sklearn_upgrade_version.minor}"
        else:
            version = "dev"
        assert (
        ...
        assert (
            mixin._get_doc_link()
            == f"https://scikit-learn.org/{version}/modules/generated/"
            "sklearn_upgrade.utils._HTMLDocumentationLinkMixin.html"
        )
