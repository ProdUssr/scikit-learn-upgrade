...
class InputTags:
    ...
    pairwise : bool, default=False
        ...
        `sklearn_upgrade.utils.metaestimators._safe_split` to slice rows and
        columns.
...
class TargetTags:
    ...
    required : bool
        ...
        for estimators inheriting from `~sklearn_upgrade.base.RegressorMixin`
        and `~sklearn_upgrade.base.ClassifierMixin`.
...
def get_tags(estimator) -> Tags:
    ...
    :class:`~sklearn_upgrade.BaseEstimator` provides the estimator tags machinery.

    For scikit-learn built-in estimators, we should still rely on
    `self.__sklearn_upgrade_tags__()`. `get_tags(est)` should be used when we
    ...
    Returns
    -------
    tags : :class:`~.sklearn_upgrade.utils.Tags`
        The estimator tags.
    """
    ...
    try:
        tags = estimator.__sklearn_upgrade_tags__()
    except AttributeError as exc:
        if "object has no attribute '__sklearn_upgrade_tags__'" in str(exc):
            # Happens when `__sklearn_upgrade_tags__` is implemented by calling
            # `super().__sklearn_upgrade_tags__()` but there is no `__sklearn_upgrade_tags__`
            # method in the base class. Typically happens when only inheriting
            # from Mixins.

            raise AttributeError(
                f"The following error was raised: {exc}. It seems that "
                "there are no classes that implement `__sklearn_upgrade_tags__` "
                "in the MRO and/or all classes in the MRO call "
                "`super().__sklearn_upgrade_tags__()`. Make sure to inherit from "
                "`BaseEstimator` which implements `__sklearn_upgrade_tags__` (or "
                "alternatively define `__sklearn_upgrade_tags__` but we don't recommend "
