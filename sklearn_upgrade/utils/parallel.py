...
def _with_config_and_warning_filters(delayed_func, config, warning_filters):
    ...
    if hasattr(delayed_func, "with_config_and_warning_filters"):
        ...
        warnings.warn(
            (
                "`sklearn_upgrade.utils.parallel.Parallel` needs to be used in "
                "conjunction with `sklearn_upgrade.utils.parallel.delayed` instead of "
...
def delayed(function):
    ...
    with `sklearn_upgrade.utils.parallel.Parallel`. The latter captures the scikit-
    learn configuration by calling `sklearn_upgrade.get_config()` in the current
    thread, prior to dispatching the first task. The captured configuration is
    then propagated and enabled for the duration of the execution of the
    delayed function in the joblib workers.

    .. versionchanged:: 1.3
       `delayed` was moved from `sklearn_upgrade.utils.fixes` to `sklearn_upgrade.utils.parallel`
       in scikit-learn 1.3.
        ...
        if not config or not warning_filters:
            warnings.warn(
                (
                    ...
                    " `sklearn_upgrade.utils.parallel.Parallel` to make it possible to"
