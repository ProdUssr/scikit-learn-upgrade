...
from sklearn_upgrade._config import config_context
...
from sklearn_upgrade.exceptions import NotFittedError, PositiveSpectrumWarning
from sklearn_upgrade.linear_model import ARDRegression

# TODO: add this estimator into the _mocking module in a further refactoring
from sklearn_upgrade.metrics.tests.test_score_objects import EstimatorWithFit
...
from sklearn_upgrade.utils._mocking import (
    MockDataFrame,
...
def test_memmap():
    ...
    with NamedTemporaryFile(prefix="sklearn_upgrade-test") as tmp:
...
def test_check_array_links_to_imputer_doc_only_for_X(input_name, retype):
    ...
    extended_msg = (
        ...
        " to consider sklearn_upgrade.ensemble.HistGradientBoostingClassifier and Regressor"
...
def test_suppress_validation():
    ...
    with pytest.raises(ValueError):
        assert_all_finite(X)
    sklearn_upgrade.set_config(assume_finite=True)
    assert_all_finite(X)
    sklearn_upgrade.set_config(assume_finite=False)
    with pytest.raises(ValueError):
        assert_all_finite(X)
