...
def test_align_api_if_sparse(sparse_interface, x, result_type):
    with sklearn_upgrade.config_context(sparse_interface=sparse_interface):
        result = sklearn_upgrade.utils._align_api_if_sparse(x)
...
def test_transform_returns_sparse(sparse_interface, result_type):
    ...
    with sklearn_upgrade.config_context(sparse_interface=sparse_interface):
        vectorizer = sklearn_upgrade.feature_extraction.text.CountVectorizer()
...
def test_function_returns_sparse(sparse_interface, result_type):
    with sklearn_upgrade.config_context(sparse_interface=sparse_interface):
        X, y = sklearn_upgrade.datasets.make_regression(n_features=2, random_state=0)
        X = sklearn_upgrade.manifold._locally_linear.barycenter_kneighbors_graph(X, 1)
        assert isinstance(X, result_type)
...
def test_estimator_property_sparse(sparse_interface, result_type):
    with sklearn_upgrade.config_context(sparse_interface=sparse_interface):
        X, y = sklearn_upgrade.datasets.make_regression(n_features=2, random_state=0)
        regr = sklearn_upgrade.linear_model.ElasticNet(random_state=0)
        regr.fit(X, y)
