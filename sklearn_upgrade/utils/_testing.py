...
from sklearn_upgrade import __file__ as sklearn_upgrade_path
...
from sklearn_upgrade.utils.validation import check_array, check_is_fitted, check_X_y
...
def ignore_warnings(obj=None, category=Warning):
    ...
    --------
    >>> import warnings
    >>> from sklearn_upgrade.utils._testing import ignore_warnings
...
def assert_allclose(
    ...
    --------
    >>> import numpy as np
    >>> from sklearn_upgrade.utils._testing import assert_allclose
    >>> x = [1e-5, 1e-3, 1e-1]
...
def check_skip_network():
    if int(os.environ.get("sklearn_upgrade_SKIP_NETWORK_TESTS", 0)):
...
def create_memmap_backed_data(data, mmap_mode="r", return_folder=False):
    ...
    """
    temp_folder = tempfile.mkdtemp(prefix="sklearn_upgrade_testing_")
...
def check_docstring_parameters(func, doc=None, ignore=None):
    ...
    if not func_name.startswith("sklearn_upgrade.") or func_name.startswith(
        "sklearn_upgrade.externals"
    ):
        return incorrect
...
def assert_docstring_consistency(
    ...
    Examples
    --------
    >>> from sklearn_upgrade.metrics import (accuracy_score, classification_report,
    ... mean_absolute_error, mean_squared_error, median_absolute_error)
    >>> from sklearn_upgrade.utils._testing import assert_docstring_consistency
    ... # doctest: +SKIP
...
def assert_run_python_script_without_output(source_code, pattern=".+", timeout=60):
    ...
    """
    fd, source_file = tempfile.mkstemp(suffix="_src_test_sklearn_upgrade.py")
    os.close(fd)
    try:
        with open(source_file, "wb") as f:
            f.write(source_code.encode("utf-8"))
        cmd = [sys.executable, source_file]
        cwd = op.normpath(op.join(op.dirname(sklearn_upgrade_path), ".."))
        env = os.environ.copy()
        try:
...
def _array_api_for_tests(array_namespace, device_name=None, dtype_name=None):
    ...
    from sklearn_upgrade.externals.array_api_compat import get_namespace
