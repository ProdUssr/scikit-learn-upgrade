...
from sklearn_upgrade.utils.sparsefuncs import sparse_matmul_to_dense
from sklearn_upgrade.utils.sparsefuncs_fast import csr_row_norms
from sklearn_upgrade.utils.validation import check_array, check_random_state
...
def fast_logdet(A):
    ...
    --------
    >>> import numpy as np
    >>> from sklearn_upgrade.utils.extmath import fast_logdet
    >>> a = np.array([[5, 1], [2, 8]])
    >>> fast_logdet(a)
...
def density(w):
    ...
    >>> from sklearn_upgrade.utils.extmath import density
...
def randomized_range_finder(
    ...
    --------
    >>> import numpy as np
    >>> from sklearn_upgrade.utils.extmath import randomized_range_finder
...
def weighted_mode(a, w, *, axis=0):
    ...
    Examples
    --------
    >>> from sklearn_upgrade.utils.extmath import weighted_mode
    >>> x = [4, 1, 4, 2, 4, 2]
    >>> weights = [1, 1, 1, 1, 1, 1]
...
def svd_flip(u, v, u_based_decision=True):
    ...
    u : ndarray
        ...
        :func:`~sklearn_upgrade.utils.extmath.randomized_svd`, with matching inner
...
# TODO(1.10): Remove
@deprecated(
    "`sklearn_upgrade.utils.extmath.stable_cumsum` is deprecated in version 1.8 and "
...
def safe_sqr(X, *, copy=True):
    ...
    Examples
    --------
    >>> from sklearn_upgrade.utils import safe_sqr
    >>> safe_sqr([1, 2, 3])
    array([1, 4, 9])
    """
