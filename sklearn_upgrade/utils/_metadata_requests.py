...
from sklearn_upgrade import get_config
from sklearn_upgrade.exceptions import UnsetMetadataPassedError
from sklearn_upgrade.utils._bunch import Bunch
...
def _raise_for_params(params, owner, method, allow=None):
    ...
    if not _routing_enabled() and (params.keys() - allow):
        raise ValueError(
            ...
            " `sklearn_upgrade.set_config`. See the User Guide"
...
class MethodMetadataRequest:
    ...
    def _route_params(self, params, parent, caller):
        ...
        -------
        params : Bunch
            A :class:`~sklearn_upgrade.utils.Bunch` of {metadata: value} which can be
...
class MetadataRequest:
    ...
    Examples
    --------
    >>> from sklearn_upgrade import set_config
    >>> set_config(enable_metadata_routing=True)
    >>> from pprint import pprint
    >>> from sklearn_upgrade.utils.metadata_routing import MetadataRequest
    ...
    def _route_params(self, *, params, method, parent, caller):
        ...
        -------
        params : Bunch
            A :class:`~sklearn_upgrade.utils.Bunch` of {metadata: value} which can be given to
...
class MethodMapping:
    ...
    Examples
    --------
    >>> from sklearn_upgrade.utils.metadata_routing import MethodMapping
...
class MetadataRouter:
    ...
    is an instance of :class:`~sklearn_upgrade.utils.metadata_routing.MethodMapping` and
    ``router`` is either a
    :class:`~sklearn_upgrade.utils.metadata_routing.MetadataRequest` or another
    :class:`~sklearn_upgrade.utils.metadata_routing.MetadataRouter` instance.
    ...
    >>> from pprint import pprint
    >>> from sklearn_upgrade import set_config
    >>> from sklearn_upgrade.feature_selection import SelectFromModel
    >>> from sklearn_upgrade.linear_model import LinearRegression
    >>> from sklearn_upgrade.utils.metadata_routing import MetadataRouter, MethodMapping
    >>> set_config(enable_metadata_routing=True)
    ...
    def add_self_request(self, obj):
        ...
        :class:`~sklearn_upgrade.utils.metadata_routing.MetadataRequest`.
    ...
    def add(self, *, method_mapping, **objs):
        ...
        **objs : dict
            ...
            :func:`~sklearn_upgrade.utils.metadata_routing.get_routing_for_object` on them.
    ...
    def route_params(self, *, caller, params):
        ...
        Returns a :class:`~sklearn_upgrade.utils.Bunch` containing the metadata that this
        ...
        -------
        params : Bunch
            A :class:`~sklearn_upgrade.utils.Bunch` of the form
...
def get_routing_for_object(obj=None):
    ...
    :class:`~sklearn_upgrade.utils.metadata_routing.MetadataRouter` or a
    :class:`~sklearn_upgrade.utils.metadata_routing.MetadataRequest` from the given input.
    ...
    obj : object
        ...
        - If the object is already a
            :class:`~sklearn_upgrade.utils.metadata_routing.MetadataRequest` or a
            :class:`~sklearn_upgrade.utils.metadata_routing.MetadataRouter`, return a copy
            of that.
        - Returns an empty :class:`~sklearn_upgrade.utils.metadata_routing.MetadataRequest`
            otherwise.
    ...
    >>> from sklearn_upgrade.pipeline import Pipeline
    >>> from sklearn_upgrade.preprocessing import StandardScaler
    >>> from sklearn_upgrade.linear_model import LogisticRegressionCV
    >>> from sklearn_upgrade.utils.metadata_routing import get_routing_for_object
    ...
    <class 'sklearn_upgrade.utils._metadata_requests.MetadataRouter'>
    >>> type(get_routing_for_object(pipe.named_steps.scaler))
    <class 'sklearn_upgrade.utils._metadata_requests.MetadataRequest'>
...
passed to the ``{method}`` method.
        ...
        with ``enable_metadata_routing=True`` (see :func:`sklearn_upgrade.set_config`).
...
this given alias instead of the original name.
        ...
        The default (``sklearn_upgrade.utils.metadata_routing.UNCHANGED``) retains the
...
REQUESTER_DOC_PARAM = """        {metadata} : str, True, False, or None, \
                    default=sklearn_upgrade.utils.metadata_routing.UNCHANGED
        ...
        def func(*args, **kw):
            ...
            if not _routing_enabled():
                raise RuntimeError(
                    ...
                    " sklearn_upgrade.set_config(enable_metadata_routing=True)."
                )
...
class _MetadataRequester:
    ...
    def _get_metadata_request(self):
        ...
        request : MetadataRequest
            A :class:`~sklearn_upgrade.utils.metadata_routing.MetadataRequest` instance.
        """
    ...
    def get_metadata_routing(self):
        ...
        routing : MetadataRequest
            A :class:`~sklearn_upgrade.utils.metadata_routing.MetadataRequest` encapsulating
            routing information.
        """
...
def process_routing(_obj, _method, /, **kwargs):
    ...
    The output of this function is a :class:`~sklearn_upgrade.utils.Bunch` that has a key for
    ...
    **kwargs : dict
    ...
    -------
    routed_params : Bunch
        A :class:`~sklearn_upgrade.utils.Bunch` of the form ``{"object_name":
    ...
    >>> import numpy as np
    >>> from sklearn_upgrade import set_config
    >>> from sklearn_upgrade.utils.metadata_routing import process_routing
    >>> from sklearn_upgrade.linear_model import Ridge
    >>> from sklearn_upgrade.feature_selection import SelectFromModel
    >>> set_config(enable_metadata_routing=True)
    >>> process_routing(
