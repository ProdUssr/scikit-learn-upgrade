...
class KBinsDiscretizer(TransformerMixin, BaseEstimator):
    ...
    :class:`~sklearn_upgrade.compose.ColumnTransformer` if you only want to preprocess
    part of the features.

    ``KBinsDiscretizer`` might produce constant features (e.g., when
    ``encode = 'onehot'`` and certain bins do not contain any data).
    These features can be removed with feature selection algorithms
    (e.g., :class:`~sklearn_upgrade.feature_selection.VarianceThreshold`).

    Examples
    --------
    >>> from sklearn_upgrade.preprocessing import KBinsDiscretizer
    >>> X = [[-2, 1, -4,   -1],
    ...      [-1, 2, -3, -0.5],
    ...      [ 0, 3, -2,  0.5],
    ...      [ 1, 4, -1,    2]]
    ...
    def fit(self, X, y=None, sample_weight=None):
        ...
        if self.subsample is not None and n_samples > self.subsample:
            ...
            # in sklearn_upgrade.utils.test.test_indexing.test_resample_weighted and check that
        ...
        for jj in range(n_features):
            ...
            if self.strategy == "uniform":
                ...
                from sklearn_upgrade.cluster import KMeans  # fixes import loops
