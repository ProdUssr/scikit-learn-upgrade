...
from sklearn_upgrade.preprocessing._encoders import OneHotEncoder
from sklearn_upgrade.utils import _array_api, check_array, metadata_routing, resample
...
from sklearn_upgrade.utils.extmath import _incremental_mean_and_var, row_norms
from sklearn_upgrade.utils.sparsefuncs import (
    incr_mean_variance_axis,
    inplace_column_scale,
    mean_variance_axis,
    min_max_axis,
)
from sklearn_upgrade.utils.sparsefuncs_fast import (
...
def scale(X, *, axis=0, with_mean=True, with_std=True, copy=True):
    ...
    StandardScaler : Performs scaling to unit variance using the Transformer
        ...
        :class:`~sklearn_upgrade.pipeline.Pipeline`).
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~sklearn_upgrade.preprocessing.scale` unless you know
        what you are doing. A common mistake is to apply it to the entire data
        *before* splitting into training and test sets. This will bias the
        model evaluation because information would have leaked from the test
        set to the training set.
        In general, we recommend using
        :class:`~sklearn_upgrade.preprocessing.StandardScaler` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
        leaking: `pipe = make_pipeline(StandardScaler(), LogisticRegression())`.

    Examples
    --------
    >>> from sklearn_upgrade.preprocessing import scale
    >>> X = [[-2, 1, 2], [-1, 0, 1]]
...
class MinMaxScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    Examples
    --------
    >>> from sklearn_upgrade.preprocessing import MinMaxScaler
...
def minmax_scale(X, feature_range=(0, 1), *, axis=0, copy=True):
    ...
    .. versionadded:: 0.17
       *minmax_scale* function interface
       to :class:`~sklearn_upgrade.preprocessing.MinMaxScaler`.
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~sklearn_upgrade.preprocessing.minmax_scale` unless you know
        what you are doing. A common mistake is to apply it to the entire data
        *before* splitting into training and test sets. This will bias the
        model evaluation because information would have leaked from the test
        set to the training set.
        In general, we recommend using
        :class:`~sklearn_upgrade.preprocessing.MinMaxScaler` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
    ...
    Examples
    --------
    >>> from sklearn_upgrade.preprocessing import minmax_scale
    >>> X = [[-2, 1, 2], [-1, 0, 1]]
...
class StandardScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    :class:`~sklearn_upgrade.decomposition.PCA` : Further removes the linear
    ...
    Examples
    --------
    >>> from sklearn_upgrade.preprocessing import StandardScaler
...
class MaxAbsScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    Examples
    --------
    >>> from sklearn_upgrade.preprocessing import MaxAbsScaler
    >>> X = [[ 1., -1.,  2.],
    ...      [ 2.,  0.,  0.],
    ...      [ 0.,  1., -1.]]
...
def maxabs_scale(X, *, axis=0, copy=True):
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~sklearn_upgrade.preprocessing.maxabs_scale` unless you know
        what you are doing. A common mistake is to apply it to the entire data
        *before* splitting into training and test sets. This will bias the
        model evaluation because information would have leaked from the test
        set to the training set.
        In general, we recommend using
        :class:`~sklearn_upgrade.preprocessing.MaxAbsScaler` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
    ...
    Examples
    --------
    >>> from sklearn_upgrade.preprocessing import maxabs_scale
    >>> X = [[-2, 1, 2], [-1, 0, 1]]
...
class RobustScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    sklearn_upgrade.decomposition.PCA : Further removes the linear correlation across
    ...
    Examples
    --------
    >>> from sklearn_upgrade.preprocessing import RobustScaler
    >>> X = [[ 1., -2.,  2.],
    ...      [ -2.,  1.,  3.],
    ...      [ 4.,  1., -2.]]
...
def robust_scale(
    ...
    *,
    ...
    RobustScaler : Performs centering and scaling using the Transformer API
        (e.g. as part of a preprocessing :class:`~sklearn_upgrade.pipeline.Pipeline`).
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~sklearn_upgrade.preprocessing.robust_scale` unless you know
        what you are doing. A common mistake is to apply it to the entire data
        *before* splitting into training and test sets. This will bias the
        model evaluation because information would have leaked from the test
        set to the training set.
        In general, we recommend using
        :class:`~sklearn_upgrade.preprocessing.RobustScaler` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
        leaking: `pipe = make_pipeline(RobustScaler(), LogisticRegression())`.

    Examples
    --------
    >>> from sklearn_upgrade.preprocessing import robust_scale
    >>> X = [[-2, 1, 2], [-1, 0, 1]]
...
class Normalizer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    Examples
    --------
    >>> from sklearn_upgrade.preprocessing import Normalizer
    >>> X = [[4, 1, 2, 2],
    ...      [1, 3, 9, 3],
    ...      [5, 7, 5, 1]]
...
def binarize(X, *, threshold=0.0, copy=True):
    ...
    Examples
    --------
    >>> from sklearn_upgrade.preprocessing import binarize
...
class Binarizer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    Examples
    --------
    >>> from sklearn_upgrade.preprocessing import Binarizer
    >>> X = [[ 1., -1.,  2.],
    ...      [ 2.,  0.,  0.],
    ...      [ 0.,  1., -1.]]
...
class KernelCenterer(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    for :class:`~sklearn_upgrade.decomposition.KernelPCA` for instance.
    ...
    See Also
    --------
    sklearn_upgrade.kernel_approximation.Nystroem : Approximate a kernel map
    ...
    Examples
    --------
    >>> from sklearn_upgrade.preprocessing import KernelCenterer
...
def add_dummy_feature(X, value=1.0):
    ...
    Examples
    --------
    >>> from sklearn_upgrade.preprocessing import add_dummy_feature
...
class QuantileTransformer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    --------
    >>> import numpy as np
    >>> from sklearn_upgrade.preprocessing import QuantileTransformer
...
def quantile_transform(
    ...
    *,
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~sklearn_upgrade.preprocessing.quantile_transform` unless
        ...
        training set.
        In general, we recommend using
        :class:`~sklearn_upgrade.preprocessing.QuantileTransformer` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
    ...
    --------
    >>> import numpy as np
    >>> from sklearn_upgrade.preprocessing import quantile_transform
    >>> rng = np.random.RandomState(0)
    >>> X = np.sort(rng.normal(loc=0.5, scale=0.25, size=(25, 1)), axis=0)
...
class PowerTransformer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    --------
    >>> import numpy as np
    >>> from sklearn_upgrade.preprocessing import PowerTransformer
    >>> pt = PowerTransformer()
    >>> data = [[1, 2], [3, 2], [4, 5]]
    >>> print(pt.fit(data))
    PowerTransformer()
...
def power_transform(X, method="yeo-johnson", *, standardize=True, copy=True):
    ...
    --------
    >>> import numpy as np
    >>> from sklearn_upgrade.preprocessing import power_transform
    >>> data = [[1, 2], [3, 2], [4, 5]]
    ...
    .. warning:: Risk of data leak.
        Do not use :func:`~sklearn_upgrade.preprocessing.power_transform` unless you
        know what you are doing. A common mistake is to apply it to the entire
        data *before* splitting into training and test sets. This will bias the
        model evaluation because information would have leaked from the test
        set to the training set.
        In general, we recommend using
        :class:`~sklearn_upgrade.preprocessing.PowerTransformer` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
