...
_global_config = {
    "assume_finite": bool(os.environ.get("sklearn_upgrade_ASSUME_FINITE", False)),
    "working_memory": int(os.environ.get("sklearn_upgrade_WORKING_MEMORY", 1024)),
    "print_changed_only": True,
    "display": "diagram",
    "pairwise_dist_chunk_size": int(
        os.environ.get("sklearn_upgrade_PAIRWISE_DIST_CHUNK_SIZE", 256)
    ),
...
def get_config():
    ...
    >>> config = sklearn_upgrade.get_config()
    >>> config.keys()
    dict_keys([...])
    """
...
def set_config(
    ...
    if array_api_dispatch is not None:
        from sklearn_upgrade.utils._array_api import _check_array_api_dispatch
...
def config_context(
    *,
    ...
    --------
    >>> import sklearn_upgrade
    >>> from sklearn_upgrade.utils.validation import assert_all_finite
    >>> with sklearn_upgrade.config_context(assume_finite=True):
    ...     assert_all_finite([float('nan')])
    >>> with sklearn_upgrade.config_context(assume_finite=True):
    ...     with sklearn_upgrade.config_context(assume_finite=False):
    ...         assert_all_finite([float('nan')])
