"""
The :mod:`sklearn_upgrade.model_selection._validation` module includes classes and
...
from sklearn_upgrade.base import clone, is_classifier
from sklearn_upgrade.exceptions import FitFailedWarning, UnsetMetadataPassedError
from sklearn_upgrade.metrics import check_scoring, get_scorer_names
from sklearn_upgrade.metrics._scorer import _MultimetricScorer
from sklearn_upgrade.model_selection._split import check_cv
from sklearn_upgrade.preprocessing import LabelEncoder
from sklearn_upgrade.utils import Bunch, _safe_indexing, check_random_state, indexable
...
from sklearn_upgrade.utils.metaestimators import _safe_split
from sklearn_upgrade.utils.parallel import Parallel, delayed
from sklearn_upgrade.utils.validation import _check_method_params, _num_samples
...
def _check_groups_routing_disabled(groups):
    if groups is not None and _routing_enabled():
        raise ValueError(
            ...
            " `sklearn_upgrade.set_config(enable_metadata_routing=True)`. When routing is"
    ...
    *,
    ...
    groups : array-like of shape (n_samples,), default=None
        ...
        .. versionchanged:: 1.4
            ...
            via ``sklearn_upgrade.set_config(enable_metadata_routing=True)``. When routing
    ...
    sklearn_upgrade.metrics.make_scorer : Make a scorer from a performance metric or
        loss function.

    Examples
    --------
    >>> from sklearn_upgrade import datasets, linear_model
    >>> from sklearn_upgrade.model_selection import cross_validate
...
def cross_val_score(
    ...
    *,
    ...
    --------
    >>> from sklearn_upgrade import datasets, linear_model
    >>> from sklearn_upgrade.model_selection import cross_val_score
    >>> diabetes = datasets.load_diabetes()
    >>> X = diabetes.data[:150]
    >>> y = diabetes.target[:150]
    >>> lasso = linear_model.Lasso()
...
def cross_val_predict(
    ...
    *,
    ...
    --------
    >>> from sklearn_upgrade import datasets, linear_model
    >>> from sklearn_upgrade.model_selection import cross_val_predict
    >>> diabetes = datasets.load_diabetes()
    >>> X = diabetes.data[:150]
    >>> y = diabetes.target[:150]
    >>> lasso = linear_model.Lasso()
    ...
    if n_classes != classes_length:
        ...
        if method == "decision_function":
            if predictions.ndim == 2 and predictions.shape[1] != classes_length:
                ...
                # it with. This case is found when sklearn_upgrade.svm.SVC is
...
def permutation_test_score(
    ...
    *,
    ...
    >>> from sklearn_upgrade.model_selection import permutation_test_score
...
def learning_curve(
    ...
    *,
    ...
    --------
    >>> from sklearn_upgrade.datasets import make_classification
    >>> from sklearn_upgrade.tree import DecisionTreeClassifier
    >>> from sklearn_upgrade.model_selection import learning_curve
...
def validation_curve(
    ...
    *,
    ...
    >>> import numpy as np
    >>> from sklearn_upgrade.datasets import make_classification
    >>> from sklearn_upgrade.model_selection import validation_curve
    >>> from sklearn_upgrade.linear_model import LogisticRegression
