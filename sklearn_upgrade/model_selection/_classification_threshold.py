...
from sklearn_upgrade.metrics._scorer import _CurveScorer, _threshold_scores_to_class_labels
from sklearn_upgrade.model_selection._split import StratifiedShuffleSplit, check_cv
from sklearn_upgrade.utils import _safe_indexing, get_tags
from sklearn_upgrade.utils._param_validation import HasMethods, Interval, RealNotInt, StrOptions
from sklearn_upgrade.utils._response import _get_response_values_binary
...
from sklearn_upgrade.utils.multiclass import type_of_target
...
class FixedThresholdClassifier(BaseThresholdClassifier):
    ...
    See Also
    --------
    sklearn_upgrade.model_selection.TunedThresholdClassifierCV : Classifier that post-tunes
        the decision threshold based on some metrics and using cross-validation.
    sklearn_upgrade.calibration.CalibratedClassifierCV : Estimator that calibrates
        probabilities.
    ...
    >>> from sklearn_upgrade.model_selection import FixedThresholdClassifier, train_test_split
...
class TunedThresholdClassifierCV(BaseThresholdClassifier):
    ...
    See Also
    --------
    sklearn_upgrade.model_selection.FixedThresholdClassifier : Classifier that uses a
        constant threshold.
    sklearn_upgrade.calibration.CalibratedClassifierCV : Estimator that calibrates
        probabilities.
    ...
    >>> from sklearn_upgrade.model_selection import TunedThresholdClassifierCV, train_test_split
    >>> X, y = make_classification(
    ...     n_samples=1_000, weights=[0.9, 0.1], class_sep=0.8, random_state=42
    ... )
    >>> X_train, X_test, y_train, y_test = train_test_split(
