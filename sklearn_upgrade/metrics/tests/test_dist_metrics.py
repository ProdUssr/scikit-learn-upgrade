...
from sklearn_upgrade.metrics._dist_metrics import (
    BOOL_METRICS,
    DEPRECATED_METRICS,
    DistanceMetric32,
    DistanceMetric64,
...
def test_cdist(metric_param_grid, X, Y, csr_container):
    ...
    for vals in itertools.product(*param_grid.values()):
        ...
        D_sklearn_upgrade = dm.pairwise(X, Y)
        assert D_sklearn_upgrade.flags.c_contiguous
        assert_allclose(D_sklearn_upgrade, D_scipy_cdist, **rtol_dict)

        D_sklearn_upgrade = dm.pairwise(X_csr, Y_csr)
        assert D_sklearn_upgrade.flags.c_contiguous
        assert_allclose(D_sklearn_upgrade, D_scipy_cdist, **rtol_dict)

        D_sklearn_upgrade = dm.pairwise(X_csr, Y)
        assert D_sklearn_upgrade.flags.c_contiguous
        assert_allclose(D_sklearn_upgrade, D_scipy_cdist, **rtol_dict)

        D_sklearn_upgrade = dm.pairwise(X, Y_csr)
        assert D_sklearn_upgrade.flags.c_contiguous
        assert_allclose(D_sklearn_upgrade, D_scipy_cdist, **rtol_dict)
...
def test_cdist_bool_metric(metric, X_bool, Y_bool, csr_container):
    ...
    D_sklearn_upgrade = dm.pairwise(X_bool, Y_bool)
    assert_allclose(D_sklearn_upgrade, D_scipy_cdist)
    ...
    D_sklearn_upgrade = dm.pairwise(X_bool_csr, Y_bool_csr)
    assert D_sklearn_upgrade.flags.c_contiguous
    assert_allclose(D_sklearn_upgrade, D_scipy_cdist)

    D_sklearn_upgrade = dm.pairwise(X_bool, Y_bool_csr)
    assert D_sklearn_upgrade.flags.c_contiguous
    assert_allclose(D_sklearn_upgrade, D_scipy_cdist)

    D_sklearn_upgrade = dm.pairwise(X_bool_csr, Y_bool)
    assert D_sklearn_upgrade.flags.c_contiguous
    assert_allclose(D_sklearn_upgrade, D_scipy_cdist)
...
def test_pdist(metric_param_grid, X, csr_container):
    ...
    for vals in itertools.product(*param_grid.values()):
        ...
        dm = DistanceMetric.get_metric(metric, X.dtype, **kwargs)
        D_sklearn_upgrade = dm.pairwise(X)
        assert D_sklearn_upgrade.flags.c_contiguous
        assert_allclose(D_sklearn_upgrade, D_scipy_pdist, **rtol_dict)

        D_sklearn_upgrade_csr = dm.pairwise(X_csr)
        assert D_sklearn_upgrade.flags.c_contiguous
        assert_allclose(D_sklearn_upgrade_csr, D_scipy_pdist, **rtol_dict)

        D_sklearn_upgrade_csr = dm.pairwise(X_csr, X_csr)
        assert D_sklearn_upgrade.flags.c_contiguous
        assert_allclose(D_sklearn_upgrade_csr, D_scipy_pdist, **rtol_dict)
...
def test_pdist_bool_metrics(metric, X_bool, csr_container):
    ...
    dm = DistanceMetric.get_metric(metric)
    D_sklearn_upgrade = dm.pairwise(X_bool)
    assert_allclose(D_sklearn_upgrade, D_scipy_pdist)

    X_bool_csr = csr_container(X_bool)
    D_sklearn_upgrade = dm.pairwise(X_bool_csr)
    assert_allclose(D_sklearn_upgrade, D_scipy_pdist)
...
def test_haversine_metric(X, Y, csr_container):
    ...
    D_sklearn_upgrade = haversine.pairwise(X, Y)
    assert_allclose(
        haversine.dist_to_rdist(D_sklearn_upgrade), np.sin(0.5 * D_reference) ** 2, rtol=1e-6
    )

    assert_allclose(D_sklearn_upgrade, D_reference)

    D_sklearn_upgrade = haversine.pairwise(X_csr, Y_csr)
    assert D_sklearn_upgrade.flags.c_contiguous
    assert_allclose(D_sklearn_upgrade, D_reference)

    D_sklearn_upgrade = haversine.pairwise(X_csr, Y)
    assert D_sklearn_upgrade.flags.c_contiguous
    assert_allclose(D_sklearn_upgrade, D_reference)

    D_sklearn_upgrade = haversine.pairwise(X, Y_csr)
    assert D_sklearn_upgrade.flags.c_contiguous
    assert_allclose(D_sklearn_upgrade, D_reference)
