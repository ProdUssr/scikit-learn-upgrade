"""
The :mod:`sklearn_upgrade.metrics.scorer` submodule implements a flexible
interface for model selection and evaluation using
arbitrary score functions.

A scorer object is a callable that can be passed to
:class:`~sklearn_upgrade.model_selection.GridSearchCV` or
:func:`sklearn_upgrade.model_selection.cross_val_score` as the ``scoring``
...
from sklearn_upgrade.metrics.cluster import (
...
from sklearn_upgrade.utils.validation import _check_response_method
...
def get_scorer(scoring):
    ...
    :func:`~sklearn_upgrade.metrics.get_scorer_names` can be used to retrieve the names
    ...
    if isinstance(scoring, str):
        try:
            ...
            raise ValueError(
                ...
                "Use sklearn_upgrade.metrics.get_scorer_names() "
...
def _check_multimetric_scoring(estimator, scoring):
    ...
    Parameters
    ----------
    estimator : sklearn_upgrade estimator instance
...
def get_scorer_names():
    ...
    These names can be passed to :func:`~sklearn_upgrade.metrics.get_scorer` to
    ...
    Examples
    --------
    >>> from sklearn_upgrade.metrics import get_scorer_names
...
def check_scoring(estimator=None, scoring=None, *, allow_none=False, raise_exc=True):
    ...
    >>> from sklearn_upgrade.metrics import make_scorer, accuracy_score, mean_squared_log_error
    >>> X, y = load_iris(return_X_y=True)
    >>> y *= -1
    ...
    if callable(scoring):
        ...
        if (
            ...
            and module.startswith("sklearn_upgrade.metrics.")
            and not module.startswith("sklearn_upgrade.metrics._scorer")
            and not module.startswith("sklearn_upgrade.metrics.tests.")
        ):
            raise ValueError(
